...
module.exports = function(
  ...
  const createDynamicLink = async linkObject => {
    try {
      ...
      const errorToReport = errorsHelper.wrapError(
        errors.UPSALES_CREATE_DYNAMIC_LINK_ERROR,
        err
      );
  ...
  const loadDynamicLinks = async () => {
    try {
      ...
      const errorToReport = errorsHelper.wrapError(
        errors.UPSALES_LOAD_DYNAMIC_LINK_ERROR,
        err
      );
      logger.error(errorToReport);
    }
  ...
  const isDynamicLinkExist = async linkObject => {
    try {
      ...
      const existingLink = links.find(element => {
        return element.href === linkObject.href;
      });
      if (existingLink) {
        return true;
      }
      ...
      const errorToReport = errorsHelper.wrapError(
        errors.UPSALES_IS_DYNAMIC_LINK_EXIST_ERROR,
        err
      );
      logger.error(errorToReport);
    }
  ...
  const createDynamicLinkIfNecessary = async linkObject => {
    try {
      ...
      const errorToReport = errorsHelper.wrapError(
        errors.UPSALES_CREATE_DYNAMIC_LINK_IF_NECESSARY_ERROR,
        err
      );
      logger.error(errorToReport);
    }
