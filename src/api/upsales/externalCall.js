...
module.exports = function(
  { integrationId, authenticationKey } = requiredInput('options'),
  ...
  const callHook = async (customerId, data) => {
    try {
      ...
      logger.info(
        '  Sending Upsales API request to execute an external hook...'
      );
      logger.debug(options);
      const upsalesResponse = await axios(options).then(result => {
        logger.info(
          '  Upsales API request to execute an external hook is sent.'
        );
        logger.debug(result);
        return result.data;
      });
      ...
      const errorToReport = errorsHelper.wrapError(
        errors.SEND_UPSALES_EXTERNAL_APP_HOOK_ERROR,
        err
      );
