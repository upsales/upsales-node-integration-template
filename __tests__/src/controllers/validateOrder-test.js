...
describe("src/controllers/validateOrder.js", () => {
  ...
  it("Should throw if missing order", async () => {
    ...
    await expect(validateOrder({ data: {} })).rejects.toThrow(
      Error("Missing order")
    );
  });
  ...
  it("Should throw if order missing client", async () => {
    ...
    await expect(
      validateOrder({ data: { obj: { probability: 100 } } })
    ).rejects.toThrow(Error("Missing client"));
  });
  ...
  it("Should throw if order client missing id", async () => {
    ...
    await expect(
      validateOrder({ data: { obj: { probability: 100, client: {} } } })
    ).rejects.toThrow(Error("Missing client"));
  });
  ...
  it("Should throw if upsales request fails", async () => {
    ...
    await expect(
      validateOrder({
        apiPath: "",
        data: { obj: { probability: 100, client: { id: 1 } } }
      })
    ).rejects.toThrow(Error("Missing client"));
  ...
  it("Should throw if upsales request fails", async () => {
    ...
    ).rejects.toThrow(Error("Missing orgno."));
  });
  ...
  it("Should return ok if client has org no", async () => {
    const validateOrder = require("../../../src/controllers/validateOrder");
    const upsales = require("@upsales/node-upsales");
    upsales.prototype.__returnNext({
      custom: [{ value: "12345678", fieldId: 1 }]
    });
  ...
  it("Should return ok if client has org no (order.client is id)", async () => {
    ...
    await expect(
      validateOrder({
        apiPath: "",
        data: { obj: { probability: 100, client: 1 } }
      })
    ).resolves.toBe("ok");
  });
  ...
  it("Should return ok if the order is not 100% probability", async () => {
    ...
    await expect(
      validateOrder({
        apiPath: "",
        data: { obj: { probability: 50, client: { id: 1 } } }
      })
    ).resolves.toBe("ok");
  });
});
