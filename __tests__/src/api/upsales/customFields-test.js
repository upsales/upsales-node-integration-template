...
const getDefaultObject = () => {
  return Object.assign(
    ...
    {
      id: 1002,
      custom: [{ fieldId: 2, value: 'AAA' }, { fieldId: 5, value: 100 }]
    }
  );
};
...
const getDefaultUpsales = (
  metadataProvider = getDefaultCustomFieldsMetadata
) => ({
  order: {
    customfields: {
      create: jest.fn(data => {
...
const getTwoStagedUpsales = (
  metadataProvider1 = getDefaultCustomFieldsMetadata,
  metadataProvider2 = getDefaultCustomFieldsMetadata
) => ({
  order: {
    customfields: {
      create: jest.fn(data => {
      ...
      list: jest
        .fn()
        .mockResolvedValueOnce(metadataProvider1())
        .mockResolvedValue(metadataProvider2())
    }
  }
});
...
describe('src/helpers/customFields.js: findFieldByAlias', () => {
  ...
  it('findFieldByAlias should find and return field by alias', () => {
    ...
    const foundField = customFields.metadata.findFieldByAlias(
      getDefaultCustomFieldsMetadata(),
      'ALIAS_10'
    );
  ...
  it('findFieldByAlias should ignore fields with absent aliases', () => {
    ...
    const customFields = new CustomFields({}, { errorsHelper, logger });
    const foundField = customFields.metadata.findFieldByAlias(
      getCustomFieldsMetadataWithAbsentAliases(),
      'ALIAS_5'
    );
  ...
  it('findFieldByAlias should ignore fields with not string aliases', () => {
    ...
    const customFields = new CustomFields({}, { errorsHelper, logger });
    const foundField = customFields.metadata.findFieldByAlias(
      getCustomFieldsMetadataWithNotStringAliases(),
      'ALIAS_5'
    );
    expect(foundField).toEqual(
      getCustomFieldsMetadataWithNotStringAliases()[4]
    );
  });
});
...
describe('src/helpers/customFields.js: getValueById', () => {
  ...
  it('getValueById should find field by ID', () => {
    const CustomFields = require('../../../../src/api/upsales/customFields');
    const customFields = new CustomFields({}, { errorsHelper, logger });
    const foundField = customFields.getValueById(
      getDefaultCustomFieldsData(),
      5
    );
...
describe('src/helpers/customFields.js: getValueByAlias', () => {
  ...
  it('getValueByAlias should find field value by alias', async () => {
    ...
    const customFields = new CustomFields(
      {},
      { upsales, errorsHelper, logger }
    );

    const foundValue = await customFields.getValueByAlias(
      getDefaultCustomFieldsData(),
      'order',
      'ALIAS_10'
    );
...
describe('src/helpers/customFields.js: setValueById', () => {
  ...
  it('setValueById should add new field to an existing custom fields list', () => {
    const CustomFields = require('../../../../src/api/upsales/customFields');
    const customFields = new CustomFields({}, { errorsHelper, logger });
    const alteredObject = customFields.setValueById(
      getDefaultObject(),
      3,
      'Test value'
    );
  ...
  it('setValueById should add new field even when custom fields array is empty', () => {
    ...
    const customFields = new CustomFields({}, { errorsHelper, logger });
    const alteredObject = customFields.setValueById(
      getObjectWithEmptyCustomFieldsArray(),
      3,
      'Test value'
    );
    expect(alteredObject).toEqual({
      id: 1002,
      custom: [{ fieldId: 3, value: 'Test value' }]
    });
  });
  ...
  it('setValueById should add new field even when custom fields list is absent', () => {
    ...
    const customFields = new CustomFields({}, { errorsHelper, logger });
    const alteredObject = customFields.setValueById(
      getObjectWithUndefinedCustomFieldsArray(),
      3,
      'Test value'
    );
    expect(alteredObject).toEqual({
...
describe('src/helpers/customFields.js: setValueByAlias', () => {
  ...
  it('setValueByAlias should add new field to an existing custom fields list', async () => {
    ...
    const alteredObject = await customFields.setValueByAlias(
      getDefaultObject(),
      'order',
      'ALIAS_3',
      'Test value'
  ...
  it('setValueByAlias should add new field even when custom fields array is empty', async () => {
    ...
    expect(alteredObject).toEqual({
      id: 1002,
      custom: [{ fieldId: 6, value: 'Test value' }]
    });
  });
...
describe('src/helpers/customFields.js: loadEntityFieldByAlias', () => {
  ...
  it('loadEntityFieldByAlias should load custom field by its alias', async () => {
    ...
    const foundField = await customFields.metadata.loadEntityFieldByAlias(
      'order',
      'ALIAS_10',
      false
    );
  ...
  it('loadEntityFieldByAlias should use cached value when acquiring same field', async () => {
    const CustomFields = require('../../../../src/api/upsales/customFields');
    const upsales = getTwoStagedUpsales(
      undefined,
      getAlteredCustomFieldsMetadata
    );
    const customFields = new CustomFields(
      {},
      { upsales, errorsHelper, logger }
    ...
    await customFields.metadata.loadEntityFieldByAlias(
      'order',
      'ALIAS_5',
      true
    );
    const secondField = await customFields.metadata.loadEntityFieldByAlias(
      'order',
      'ALIAS_5',
      true
    );
...
describe('src/helpers/customFields.js: loadEntityFields', () => {
  ...
  it('loadEntityFields should use cache', async () => {
    ...
    await customFields.metadata.loadEntityFields('order', true);
    const fieldsMapSecond = await customFields.metadata.loadEntityFields(
      'order',
      true
    );
  ...
  it('loadEntityFields should not cache data if caching is switched off', async () => {
    ...
    await customFields.metadata.loadEntityFields('order', false);
    const fieldsMapSecond = await customFields.metadata.loadEntityFields(
      'order',
      true
    );
...
describe('src/helpers/customFields.js: ensureFieldsExist', () => {
  ...
  it('ensureFieldsExist should find several existing fields for different entities', async () => {
    ...
    const result = await customFields.metadata.ensureFieldsExist(
  ...
  it('ensureFieldsExist should not create new field if existing is found (several fields for different entities)', async () => {
    ...
    await customFields.metadata.ensureFieldsExist(getDefaultFieldsList(), true);
  ...
  it('ensureFieldsExist should create several missing fields for different entities', async () => {
    ...
    expect(upsales.order.customfields.create.mock.calls).toEqual(
      getMissingFieldsOrderCreation()
    );
    expect(upsales.account.customfields.create.mock.calls).toEqual(
      getMissingFieldsAccountCreationCalls()
    );
  });
  ...
  it('ensureFieldsExist should find several existing fields (alias case insensitive) for different entities', async () => {
    ...
    const result = await customFields.metadata.ensureFieldsExist(
      getCaseInsensitiveAliasFieldsList(),
      true
    );
  ...
  it('ensureFieldsExist should keep alias case when creating fields', async () => {
    ...
    const result = await customFields.metadata.ensureFieldsExist(
      getKeepCaseOfAliasOnCreationFieldsList(),
      true
    );
    expect(result).toEqual(getKeepCaseOfAliasOnCreationFieldsResult());
    expect(upsales.order.customfields.create.mock.calls).toEqual(
      getKeepCaseOfAliasOnCreationFieldsOrderCreation()
    );
    expect(upsales.account.customfields.create.mock.calls).toEqual(
      getKeepCaseOfAliasOnCreationFieldsAccountCreationCalls()
    );
  });
  ...
  it('ensureFieldsExist should safely ignore absent field desсription', async () => {
    ...
    const result = await customFields.metadata.ensureFieldsExist(
      getIgnoreAbsentFieldDescriptionFieldsList(),
      true
    );
    expect(result).toEqual(getIgnoreAbsentFieldDescriptionFieldsResult());
    expect(upsales.order.customfields.create.mock.calls).toEqual(
      getIgnoreAbsentFieldDescriptionFieldsOrderCreation()
    );
    expect(upsales.account.customfields.create.mock.calls).toEqual(
      getIgnoreAbsentFieldDescriptionFieldsAccountCreationCalls()
    );
  });
  ...
  it('ensureFieldsExist should safely ignore absent entity type', async () => {
    ...
    const result = await customFields.metadata.ensureFieldsExist(
      getIgnoreAbsentEntityTypeFieldsList(),
      true
    );
    expect(result).toEqual(getIgnoreAbsentEntityTypeFieldsResult());
    expect(upsales.order.customfields.create.mock.calls).toEqual(
      getIgnoreAbsentEntityTypeFieldsOrderCreation()
    );
    expect(upsales.account.customfields.create.mock.calls).toEqual(
      getIgnoreAbsentEntityTypeFieldsAccountCreationCalls()
    );
  });
